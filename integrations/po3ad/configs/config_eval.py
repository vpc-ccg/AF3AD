...
def get_parser():
    ...
    parser.add_argument('--num_works', '--num_workers', dest='num_works', type=int, default=4,
    ...
    parser.add_argument('--validation', action='store_true', default=False, help='Whether to verify the validation set')
    ...
    # Offset prediction head architecture (must match training config)
    parser.add_argument('--offset_head_variant', type=str, default='baseline',
                        choices=['baseline', 'deep', 'residual', 'multi_head', 'attention'],
                        help='Architecture variant for offset prediction head (must match training)')
    parser.add_argument('--offset_hidden_dim', type=int, default=64,
                        help='Hidden dimension for offset prediction head')
    parser.add_argument('--offset_num_layers', type=int, default=3,
                        help='Number of layers in offset prediction head')
    parser.add_argument('--offset_dropout', type=float, default=0.0,
                        help='Dropout probability in offset prediction head')
    parser.add_argument('--offset_attention_reduction', type=int, default=4,
                        help='Reduction ratio for attention module')

     # Real3D-AD test set downsampling
    parser.add_argument('--test_downsample_enabled', action='store_true', default=False,
                        help='Enable voxel downsampling for Real3D-AD test set')
    parser.add_argument('--test_downsample_voxel_size', type=float, default=0.001,
                        help='Voxel size for test set downsampling (default: 0.005)')
